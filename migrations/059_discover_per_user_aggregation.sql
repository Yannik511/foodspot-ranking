-- Migration 059: Aggregation = EINE Bewertung pro Person je Spot.
--
-- Bisher: count über foodspots-Zeilen / ratings_count → ein Nutzer, der denselben
-- Spot privat UND geteilt bewertet hat, zählte doppelt.
--
-- Jetzt: Einzelbewertungen aus BEIDEN Quellen zusammenführen —
--   * geteilt: foodspot_ratings.score (je user_id)
--   * privat : foodspots.rating       (nur Zeilen OHNE foodspot_ratings)
-- Pro (canonical_key, user_id) den Durchschnitt bilden → EINE Bewertung je Person.
-- Pro canonical_key: Ø der Personen-Bewertungen, Anzahl = DISTINCT Personen.
--
-- Beispiel: ich privat(8) + ich geteilt(9) + 6 andere → 7 Bewertungen (nicht 8),
-- meine zählt als (8+9)/2 = 8,5.
--
-- Nur CREATE OR REPLACE der drei Aggregat-Funktionen (Signaturen unverändert).
-- Produkt-Fuzzy (Tippfehler) ist NICHT Teil dieser Migration (separater Schritt 2).
...
-- =====================================================================
-- get_discover_friends_recent (Ø/Anzahl = Personen-Aggregat, Anzeige pro Eintrag)
-- =====================================================================
